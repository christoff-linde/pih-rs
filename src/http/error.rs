...
/// message in a plain text format, or a JSON body in the case of `UnprocessableEntity`.
...
pub enum Error {
    ...
    /// Return `422 Unrpocessable Entity`
    ///
    /// This also serialises the `errors` map to JSON to satisfy the requirement for
    ...
    ///
    /// Via the generated `From<sqlx::Error> for Error` impl, this allows using `?` with
    /// database calls in handler functions without a manual mapping step.
    ///
    /// I highly recommend creating an error type like this if only to make handler function
    /// code nicer.
    ///
    /// The actual error message isn't returned to the client for security reasons and should
    /// be logged instead.
